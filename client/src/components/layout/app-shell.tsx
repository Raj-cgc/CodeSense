...
export function AppShell({
  ...
  return (
    <SidebarProvider className="h-dvh max-h-dvh overflow-hidden">
      <Sidebar variant="inset" collapsible="icon" className="h-dvh max-h-dvh">
        <SidebarHeader>
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton
              ...
              >
                ...
                <div className="grid flex-1 text-left text-sm leading-tight">
                  <span className="truncate font-semibold">CodeSense</span>
      ...
      <SidebarInset className="h-dvh max-h-dvh flex flex-col overflow-hidden">
        {!hideHeader && (
          <header className="sticky top-0 z-20 flex h-16 shrink-0 items-center gap-3 border-b border-border/80 bg-background/85 px-6 backdrop-blur-md">
...
export function BrandMark({ className }: { className?: string }) {
  return (
    ...
    >
      <DevPilotIcon className="size-8 rounded-[10px]" />
      <span className="font-heading text-[1.05rem] leading-none">CodeSense</span>
    </div>
  );
}
...
export function GhostButtonLink({
  ...
  return (
    <Button variant="ghost" size="sm" className={className} render={<Link to={href} />}>
      {children}
    </Button>
  );
}
